...
export function DepartmanlarScreen(props: { user: any; abilities: any; onOpenMenu: () => void }) {
  const actions = props.abilities?.actions?.Departmanlar ?? props.abilities?.Actions?.Departmanlar ?? {};
  ...
  const refresh = async (force?: boolean) => {
    ...
    setError(null);
    try {
      const resp = await organizasyonService.listDepartmanlar({ forceRefresh: !!force });
  ...
  const openEdit = async (id: number) => {
    ...
    setFormVisible(true);
    try {
      const resp = await organizasyonService.getDepartman(id);
  ...
  const submit = async () => {
    ...
    const ad = (fAd ?? "").trim();
    if (!ad) {
      showPopup("error", "Departman adı boş olamaz.");
      return;
    }
    setSaving(true);
    try {
      if (!isEdit) {
        const resp = await organizasyonService.createDepartman({ ad, aciklama: (fAciklama ?? "").trim() || "" });
        if (!resp?.success) throw new Error(resp?.message || "İşlem başarısız.");
        showPopup("success", resp?.message || "Departman kaydedildi.");
      } else {
        const resp = await organizasyonService.updateDepartman(fId, { ad, aciklama: (fAciklama ?? "").trim() || "" });
        if (!resp?.success) throw new Error(resp?.message || "İşlem başarısız.");
        showPopup("success", resp?.message || "Departman güncellendi.");
      }
  ...
  const doDelete = async () => {
    ...
    setDelSaving(true);
    try {
      const resp = await organizasyonService.deleteDepartman(delId);
      if (!resp?.success) throw new Error(resp?.message || "İşlem başarısız.");
      showPopup("success", resp?.message || "Departman silindi.");
      setDelVisible(false);
      await refresh();
    } catch (e: any) {
      showPopup("error", e?.message || "Hata oluştu.");
  ...
  const renderTopBar = () => (
    <>
      <PageHeader
        ...
        rightA11yLabel2={canCreate ? "Departman ekle" : "Bildirimler ve hesap"}
      />
      {quickMenu.modal}
    </>
  );
      ...
      <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingBottom: 24 }}>
        ...
        <View className="mt-4">
          {!filtered.length ? (
            ...
            filtered.map((r) => (
              <View key={`d_${r.id}`} className="mb-3 bg-white rounded-2xl border border-[#e2e8f0] overflow-hidden">
          ...
          <TouchableOpacity className="flex-1 bg-black/50 justify-center px-6" activeOpacity={1} onPress={() => setFormVisible(false)}>
            <TouchableOpacity activeOpacity={1} onPress={() => {}}>
              <View className="bg-white rounded-2xl overflow-hidden">
                <View className="px-4 py-3 border-b border-[#f1f5f9] flex-row items-center justify-between">
                  <Text className="text-[#0f172a] font-extrabold">{isEdit ? "Departman Düzenle" : "Yeni Departman"}</Text>
                ...
                <ScrollView style={{ maxHeight: 560 }} contentContainerStyle={{ padding: 16 }}>
                  {isEdit ? (
                    <>
                      <Text className="text-[#64748b] font-semibold">Departman Id</Text>
                  ...
                  <Text className={`${isEdit ? "mt-4" : ""} text-[#64748b] font-semibold`}>Departman Adı *</Text>
                  <TextInput
                    value={fAd}
                    onChangeText={setFAd}
              ...
              <View className="bg-white rounded-2xl overflow-hidden">
                ...
                <View className="p-4">
                  <Text className="text-[#334155] font-semibold">
                    "{delLabel}" departmanını silmek istiyor musunuz?
                  </Text>
                </View>
                <View className="p-4 border-t border-[#f1f5f9] flex-row gap-2">
                  <TouchableOpacity
